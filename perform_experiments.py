...
from prompts import EXPERIMENT_PLAN_PROMPT, EXPERIMENT_PROMPT
...
class CodeContextBuilder(BaseContextBuilder):
    def build_context(self, system, request_text, active_tasks_info, finished_tasks_info, workspace_tree, hardware_status):
        ...
        context += "【当前已经获得的数据】\n"
        with open(system.data_record_txt, "r", encoding="utf-8", errors="ignore") as f:
            context += f.read()
...
async def run_experiments(workspace_dir, user_request, settings):
    ...
    system = GenerateCodeSystem(workspace_dir, settings, interrupt_event, user_reqs, student_planner_prompt=EXPERIMENT_PLAN_PROMPT, teacher_critic_prompt=None)
    ...
    system.data_record_txt = os.path.join(workspace_dir, "recorded_data.txt")
